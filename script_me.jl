...
# TODO: write a dedicated loop, where we do 1-site VUMPS + dedicated bond expansion
alg = VUMPS(; maxiter = 1, tol = tol_warmup, verbosity = verbosity_warmup)
alg_expand = OptimalExpand(; trunc = truncrank(chi_warmup), verbosity = verbosity_warmup)
alg_cut = SvdCut(; trunc = truncrank(chi_warmup), verbosity = verbosity_warmup)


psi1, envs, err = with_logger(tracker) do
    find_groundstate(
        psi1, H_FCI_shifted,
        IDMRG2(; trunc = truncrank(chi_warmup), maxiter = maxiter_warmup, tol = tol_warmup, verbosity = 10)
    )
end

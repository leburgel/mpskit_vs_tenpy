...
function bench_eigsolve(;
    ...
    # Built by hand rather than via `Defaults.alg_eigsolve` because that
    # constructor exposes no `orth`, and the orthogonalizer is a knob under test
    # (FINDINGS 5.4 found single-pass `ModifiedGramSchmidt` was one of three that
    # combined for 1.77x). With the defaults above this is **bit-identical** to
    # `Defaults.alg_eigsolve(; krylovdim = 16, maxiter = 5, eager = true,
    # ishermitian = true, tol_factor = 1e0, tol_max = 1e-2)`, which is what every
    # run up to 2026-09-08 15:26 used:
    #
    #   DynamicTol{Lanczos{ModifiedGramSchmidt2, Float64}}(
    #       Lanczos(ModifiedGramSchmidt2(), 16, 5, 1.0e-10, true, 0),
    #       1.0e-14, 0.01, 1.0)
    #
    # so switching `orth` changes that one field and nothing else. `ishermitian`
    # is implicit: `Lanczos` is the Hermitian solver.
    alg = Lanczos(; tol, maxiter, eager, krylovdim, orth, verbosity)

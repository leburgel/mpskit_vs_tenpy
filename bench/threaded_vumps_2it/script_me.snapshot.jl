...
cap_blas_threads()
# Unit-cell parallelism is implicit: MPSKit sets its scheduler to `SerialScheduler()`
# when `Threads.nthreads() == 1` and `DynamicScheduler()` otherwise (defaults.jl:114),
# and VUMPS's `localupdate` and `envs` stages both `tforeach` over the unit cell with
# it. So `julia -t <sites>` is all that is needed -- printed here so the log records
# which regime the run was actually in.
